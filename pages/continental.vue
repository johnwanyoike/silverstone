    ...
    <span>
        <div class="page-header header-filter header-small" data-parallax="true" style=" background-image: url('/img/kit/pro/car2.jpg'); ">
            <div class="container">
                <div class="row">
                    <div class="col-md-6">
                        <img src="/img/contilogo.png" alt="" width="350">
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-md-8 col-sm-12 ml-auto mr-auto text-center">
                            <ul class="nav nav-pills nav-pills-warning">
                                ...
                                <li class="nav-item mr-auto ml-auto">
                                    <a style="font-size: 14px;" @click.prevent='servicedata(2)' :class="[AboutContinental == true ? 'active show' : '']" class="nav-link" href="#pillAbout" data-toggle="tab">About Continental</a>
                                </li>
                                <li class="nav-item mr-auto ml-auto">
                                    <a style="font-size: 14px;" @click.prevent='servicedata(3)' :class="[WhyContinental == true ? 'active show' : '']" class="nav-link" href="#pillWhy" data-toggle="tab">Why Continental tyres</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12 ml-auto mr-auto">
                            <div class="tab-content tab-space">
                                <template v-if="AvailablePatterns">
                                    ...
                                    <template v-else>
                                        <h3 class="mt-0 mb-1 title text-warning text-center">Continental Patterns</h3>
                                        <div class="row">
                                ...
                                <template v-if="AboutContinental">
                                    <h3 class="title text-warning text-center m-0">About Continental.</h3>
                                    <div class="row" v-for="dt in aboutconti" :key="dt.id">
                                ...
                                <template v-if="WhyContinental">
                                    <h3 class="mt-0 title text-warning text-center">Why Continental tyres</h3>
                                    <div class="related-products" v-for="why in whyconti" :key="why.id">
...
export default {
    ...
    computed: {
        ...mapGetters({
            ...
            aboutconti: 'contidata/aboutconti',
            whyconti: 'contidata/whyconti',
            ...
            selectfeature: 'contifeatures/selectfeature'
        }),
        fourbyfour: function(){
            var fourbyfour = [];
    ...
    methods: {
        ...
        brandWidthCallback(event){
            if (event.target.value != null) {
                ...
                this.getBrandWidth({'brand':'CONTI', 'width': event.target.value})
            }
        },
        brandRatioCallback(event){
            if (event.target.value != null) {
                ...
                this.getBrandRatio({'brand':'CONTI', 'width':this.selectbrandwidth, 'ratio':event.target.value})
            }
        },
        brandDiameterCallback(event){
            if (event.target.value != null) {
                this.searchingbrand = true
                this.clearbranddata()
                this.getBrandAlldata({'brand':'CONTI', 'width':this.selectbrandwidth, 'ratio':this.selectbrandratio, 'diameter':event.target.value})
    ...
    mounted() {
        ...
        if (this.$route.name == 'continental') {
            this.clearbranddata()
            this.setUniqueConti("CONTI");
            this.setUniqueBrandWidth("CONTI");
        }else if(this.$route.name == 'hankook'){
            this.setUniqueConti(this.$route.params.brand);
        }
        this.AvailablePatterns = true
        this.fours=true
    },
